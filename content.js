(() => {
  ...
  const MessageHandler = {
      ...
      try {
        switch (type) {
          case 'selector': {
            ...
            sendResponse({ success: true, data, url: window.location.href, timestamp: new Date().toISOString() });
            break;
          }
          case 'table': {
          ...
          case 'productDetail': {
            Scraper.scrapeProductDetail().then(data => {
              if (data && (data.sku || data.name)) {
                // Đảm bảo data là array hoặc object
                const resultData = Array.isArray(data) ? data : [data];
                sendResponse({ success: true, data: resultData, url: window.location.href, timestamp: new Date().toISOString() });
              } else {
                log(`Không có dữ liệu từ scrapeProductDetail: ${JSON.stringify(data)}`, '⚠️');
                sendResponse({ success: false, error: 'Không tìm thấy dữ liệu sản phẩm', data: [] });
              }
            }).catch(error => {
          ...
          case 'productsFromAPI':
            ...
            return true;
          case 'callAPI':
            // Generic API call - use Long Châu API as fallback
            (API?.scrapeLongChau(options) || Promise.resolve([])).then(data => {
              sendResponse({ success: true, data, url: window.location.href, timestamp: new Date().toISOString() });
            }).catch(error => {
              sendResponse({ success: false, error: error.message });
            });
    ...
    handleGetPageInfo: (sendResponse) => {
      sendResponse({
        ...
        description: Utils.safeQuery('meta[name="description"]')?.content || '',
    ...
    handleTestSelector: (request, sendResponse) => {
      ...
      try {
        const elements = Utils.safeQueryAll(selector);
        const sample = elements.slice(0, 5).map(el => {
          const href = el.href || Utils.safeQuery('a', el.closest('div, article, li'))?.href || '';
          const card = el.closest('div, article, li') || el;
          return {
            name: Utils.getText(Utils.safeQuery('h1, h2, h3, [class*="title"], [class*="name"]', card), 50) || 
                  Utils.getText(el, 50),
            href: href,
            hasPrice: !!Utils.safeQuery('[class*="price"]', card),
            hasImage: !!(Utils.safeQuery('img', card) || Utils.safeQuery('img', el))
          };
        });
  ...
  const HighlightManager = {
    ...
    highlight: (element) => {
      ...
      HighlightManager.elements.push(element);
    },

    clear: () => {
      HighlightManager.elements.forEach(el => {
        el.style.outline = '';
    ...
    highlightBySelector: (selector) => {
      ...
      try {
        const elements = Utils.safeQueryAll(selector);
        elements.forEach(el => HighlightManager.highlight(el));
